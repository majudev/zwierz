...
function MentorShowTrial({ mode }: Props): JSX.Element {
  ...
  const refreshLogbook = async function(){
    const response = await fetch(process.env.REACT_APP_API_URL + "/trial/logbook/" + userId + "/" + type, {
      method: "GET",
      mode: 'same-origin',
    });
    if(!response.ok){
...
export default MentorShowTrial;
