...
if(isset($_POST["email"]) && isset($_POST["password"]) && isset($_POST["captcha"]) && validate($_POST["email"], $_POST["password"], $_POST["captcha"])){
	if($_POST["captcha"] == $_SESSION["captcha"]){
		...
		if($result && mysqli_num_rows($result) > 0){ //search if user already registered
			...
			$mailresult = send_mail(array($_POST["email"]), NULL, array(), $config["activation_mail"]["subject"], str_replace("%ACTIVATION_LINK%", "https://".$_SERVER["HTTP_HOST"].$config["base_url"]."/api/user/activate.php?ui=yes&confirmation=".$activation_key, $config["activation_mail"]["body"]));
